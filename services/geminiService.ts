...
const continuationSchema: Schema = {
  ...
  properties: {
    chapterTitle: {
      type: Type.STRING,
      description: "The title of the current chapter, e.g., 'Chapter 1: The Fog'"
    },
    storyText: {
      type: Type.STRING,
    ...
    choices: {
      ...
      items: {
        ...
        properties: {
          ...
          type: { type: Type.STRING, description: "Either 'Logical' or 'Unexpected'" }
        },
        required: ["text", "type"]
      }
    }
...
export const continueStory = async (
  ...
  try {
    ...
    recentHistory.forEach(h => {
      contextString += `${h.role === 'user' ? 'User Choice' : 'Story Segment'}: ${h.text}\n`;
    });
